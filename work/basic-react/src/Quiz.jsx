        ...
        <div className="quiz-panel">
            {step + 1 <= quizs.length ?
                (<div>
                    ...
                    <Question question={quizs[step].question} />
                    <Answer
                    ...
                    <button
                        className="next-step"
                        disabled={clickedAnswer && quizs.length >= step + 1 ? false : true}
                        onClick={() => nextStep(step)}>NEXT</button>
                </div>) : (
                    <div className="final-page">
                        <h1>You are done!</h1>
                        <p>Your score is: {score} of {quizs.length}</p>
                        <button className="play-again" onClick={() => resetPlay()}>PLAY AGAIN</button>
                    </div>
                )
            }
        </div>
